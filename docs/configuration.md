...
| `OTEL_LOG_TOOL_DETAILS`    | No       | `1`                             | Set to `0` to exclude tool parameters from telemetry events and tool content from trace spans (only tool name is recorded). Requires Grafana Cloud telemetry — see [Telemetry](telemetry.md).                                                       |
| `OTEL_RESOURCE_ATTRIBUTES` | No       | _(auto: Fly identity)_          | Comma-separated `key=value` pairs added to all metrics, events, and traces. `fly.app_name` and `fly.machine_id` are auto-injected; operator values are appended. Requires Grafana Cloud telemetry — see [Telemetry](telemetry.md).                  |
